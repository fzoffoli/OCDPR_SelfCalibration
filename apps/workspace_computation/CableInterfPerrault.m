...
function interf_detected = CableInterfPerrault(cdpr_v,cdpr_p)
    ...
    for cIdx=1:length(cableComb)
        ...
        rho_ij = Anti(-cdpr_v.cable(cabCouple(1)).pos_BA_glob)*(-cdpr_v.cable(cabCouple(2)).pos_BA_glob);
        a_ij = cdpr_v.cable(cabCouple(2)).pos_PA_glob-cdpr_v.cable(cabCouple(1)).pos_PA_glob;
        b_ij = cdpr_v.cable(cabCouple(2)).pos_BA_glob-cdpr_v.cable(cabCouple(1)).pos_BA_glob;
        if abs(rho_ij'*a_ij)<eps
            Rho_ij = [-cdpr_v.cable(cabCouple(1)).pos_BA_glob cdpr_v.cable(cabCouple(2)).pos_BA_glob];
            d_ij = pinv(Rho_ij)*a_ij;
...
%     end
    ...
    for i=1:cdpr_p.n_cables
        for j=1:length(V_shrink)
            ...
            r_ij_s=P(V_shrink(j,1),:)'-cdpr_v.cable(i).pos_PA_glob;
            r_ij_t=P(V_shrink(j,2),:)'-cdpr_v.cable(i).pos_PA_glob;
