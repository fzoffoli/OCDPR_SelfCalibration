function F = FitnessFunSwivelAHRSMotor(cdpr_v,cdpr_p,Z,k,method)
...
delta_theta_motor = zeros(cdpr_p.n_cables,k);
delta_sigma = zeros(cdpr_p.n_cables,k);
delta_yaw = zeros(k,1);
roll = zeros(k,1);
pitch = zeros(k,1);
[~,J] = CostFunSelfCalibrationSwivelAHRSMotor(cdpr_v,cdpr_p,X,k,delta_theta_motor,delta_sigma,roll,pitch,delta_yaw);
